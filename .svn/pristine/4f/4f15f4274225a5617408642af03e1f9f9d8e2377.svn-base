	...
	<div class="container card-body p-0">
		<div class="border border-top-0 border-200 d-flex">
			<a class="col-2 btn btn btn-light title">제목</a> <input
				...
				id="emlTtl" type="text" value="">
		</div>
		<div class="border border-top-0 border-200 d-flex">
			<a class="col-2 btn btn-light">받는 사람</a> 
			<input class="form-control border-0 rounded-0 outline-none px-x1 to-email" id="rcptn" type="email" name="receiver" value=""/>
...
<!-- 			</button> -->
		...
		<div class="min-vh-50">
			<textarea class="tinymce d-none" name="brdContent" id="emlCn"
				data-tinymce="data-tinymce" name="content">
				</textarea>
		</div>
	</div>
...
<button class="btn btn-outline-primary" id="auto-1">자동완성1</button>
<button class="btn btn-outline-primary" id="auto-2">자동완성2</button>
...
<script>
	...
	$('#auto-1').on('click',()=>{
		$('#emlTtl').val("고객사 담당자 정보 보내드립니다.");
		$('#rcptn').val('ikaros9313@gmail.com');
		let content = '이번에 출장가시는 고객사 담당직원 명함입니다 확인 부탁드립니다!'
		tinymce.get('emlCn').setContent(content);
	});
	
	$('#auto-2').on('click',()=>{
		$('#emlTtl').val("입사 축하드립니다.");
		$('#rcptn').val('ikaros9313@gmail.com');
		let content = '8월 4일부터 출근하시면 되시며 사원번호는 NAVER_2023080001이며, 임시비밀번호는 java입니다. 접속하셔서 원하시는 비밀번호로 바꾸세요.'
		tinymce.get('emlCn').setContent(content);
