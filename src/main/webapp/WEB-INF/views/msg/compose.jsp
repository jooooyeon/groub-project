...
<c:set value="${msg.userId}" var="userId"/>
<form class="card col-lg-8" action="/msg/send?${_csrf.parameterName}=${_csrf.token}" method="post">
	...
	<div class="card-body p-0">
		...
		<div class="border border-top-0 border-200 d-flex">
			<a class="col-3 btn btn-light to">받는 사람</a> <input
				class="form-control border-0 rounded-0 outline-none px-x1"
				id="receiver" type="text" name="receiver" aria-describedby="" value="${userId}"
				placeholder="To">
...
<script>
	...
	$('#auto-1').on('click',()=>{
		$('#receiver').val('NAVER_2014030001');
		$('#msg-title').val('휴가 기안 올렸습니다.');
		tinymce.get('msg-text').setContent('여름휴가 보내주세요');
	});
	
	$('#auto-2').on('click',()=>{
		$('#receiver').val('NAVER_2023060008');
		$('#msg-title').val('휴가 기안 승인했습니다');
		tinymce.get('msg-text').setContent('네 휴가 잘다녀오세요.');
	});
</script>
<script src="/resources/js/msg-js/msg-compose.js"></script>