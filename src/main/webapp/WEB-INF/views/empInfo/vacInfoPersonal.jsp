...
<div class="container">
	<div class="row">
		...
		<table class="table table-bordered align-middle text-align">
			...
			<tbody>
				<tr>
					<td rowspan="4" class=" text-center">${vacMainList.totalVacGrntDays}</td>
					<td class=" text-center">${vacMainList.usedDays}</td>
					<td class=" text-center">${vacMainList.totalVacGrntDays - vacMainList.usedDays}</td>
				</tr>
			</tbody>
		</table>
		</div>
					...
					<tbody id="dataRows">
						...
						<c:forEach var="usedVac" items="${detailUsedVac}" varStatus="stat">
							<tr id="currentYearData">
								...
								<td class="text-center">${usedVac.vacTpNm}</td>
								<td class="text-center"><fmt:formatDate value="${usedVac.bgngDt}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center"><fmt:formatDate value="${usedVac.endDt}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center">${usedVac.cnyDay}</td>
				...
				<table class="table table-bordered align-middle text-align">
					...
					<tbody>
						...
						<c:forEach var="grantList" items="${grantList}" varStatus="stat">
							<tr>
								<td class="text-center">${stat.count}</td>
								<td class="text-center"><fmt:formatDate value="${grantList.vacGrntYmd }" pattern="yyyy-MM-dd" /></td>
								<td class="text-center"><fmt:formatDate value="${grantList.vacGrntYmd }" pattern="yyyy-MM-dd" />
									 ~ <fmt:formatDate value="${grantList.vldYmd }" pattern="yyyy-MM-dd" /></td>
								<td class="text-center">${grantList.vacGrntDays }</td>
								<td class="text-center">${grantList.vacGrntExpln }</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
        ...
        $('select[name="myVacYear"]').on('change', function() {
	    	...
	    	$.ajax({
          	...
          	mytbody.append(header);
					...
					row.append("<td class='text-center'>" + item.vacTpNm + "</td>");
					row.append("<td class='text-center'>" + formatDate(item.bgngDt) + "</td>");
					row.append("<td class='text-center'>" + formatDate(item.endDt) + "</td>");
					row.append("<td class='text-center'>" + item.cnyDay + "</td>");
